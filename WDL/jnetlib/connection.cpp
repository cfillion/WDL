...
void JNL_Connection::connect(JNL::SOCKET s, struct sockaddr_in *loc)
{
  close(1);
  m_socket=s;
  m_remote_port=0;
...
void JNL_Connection::run(int max_send_bytes, int max_recv_bytes, int *bytes_sent, int *bytes_rcvd)
{
  ...
  switch (m_state)
  {
    case STATE_RESOLVING:
      ...
      else if (JNL::last_error()!=JNL::_EINPROGRESS)
      {
    ...
    case STATE_CLOSING:
      if (m_send_len>0 && bytes_allowed_to_send>0)
      {
        ...
        if (len > 0)
        {
          ...
          if (res==-1 && JNL::last_error() != JNL::_EWOULDBLOCK)
          {            
...
//              m_state=STATE_CLOSED;
      ...
      if (m_recv_len<m_recv_buffer.GetSize())
      {
        ...
        if (len>0)
        {
          ...
          if (res == 0 || (res < 0 && JNL::last_error() != JNL::_EWOULDBLOCK))
          {        
            m_state=STATE_CLOSED;
            break;
          }
...
{
  if (m_socket==JNL::_INVALID_SOCKET) return 0;
  struct sockaddr_in sin;
  memset(&sin,0,sizeof(sin));
  JNL::socklen_t len=16;
